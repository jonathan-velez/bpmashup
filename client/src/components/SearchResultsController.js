...
const SearchResultsController = ({ match = {}, searchEverything, searchResults = {}, isLoading = false }) => {
  const { searchTerm } = match.params;
  const { artists = [], labels = [], releases = [], tracks = [] } = searchResults;
  ...
  // const searchResultsGroup = Object.values(searchResults);
  // let resultCount = 0;
  // for (let result of searchResultsGroup) {
      ...
      {tracks.length > 0 &&
        <Grid.Row>
          <Grid.Column>
            ...
            <ResponsiveTrackListing trackListing={tracks} isPlaylist={false} isLoading={isLoading} page={DEFAULT_PAGE} perPage={getPerPageSetting()} />
          </Grid.Column>
        </Grid.Row>
      }
    </Grid>
